<?php if (strlen($this->session->userdata('passTok')) == 200) { ?>
    ...
    <body oncontextmenu="return false" onbeforeprint="return false">
        ...
        <div class="" id="element" oncontextmenu="return false" onbeforeprint="return false" ondragstart="return false">
				...
				<div class="row">
					...
					<div class="scroll-container">
						...
						foreach ($listFig as $value) {
							...
							$imageWidth = '70px';//count($listFig) > 3 ? '80%' : '40%'; // Set width to 80% if more than 2 images, otherwise 30%
							$imageHeight = '70px'; //count($listFig) > 3 ? '40%' : '30%'; // Set width to 80% if more than 2 images, otherwise 30%
							$objectFit = count($listFig) > 3 ? '' : 'object-fit: initial;'; // Set width to 80% if more than 2 images, otherwise 30%

							echo '<div class="image-container" style="position: relative; display: inline-block; padding-top: 1rem;">';
          ...
          class="slider-image zoomable" onclick="myFunction(this);" >' .
								'<div><a href="#" class="btn">' . $value['TitreFigure'] . '</a></div>';
                ...
                <i class="fa fa-trash-alt" title="' . htmlspecialchars($this->lang->line('actionSupp')) . '" style="font-size: 0.8rem;"></i>
							...
							if (count($listFig) > 1 && ($counter + 1) < count($listFig)) {
								echo '<i id="min_' . base64_encode($value['IDFigure']) . '" class="fa fa-minus" style="padding-right: 1rem; opacity: 0; font-size: 0rem;"></i>';
							}
