<?php if ((strlen($this->session->userdata('passTok')) == 200) && ($this->session->userdata('EstAdmin') == 1)) { ?>
                    ...
                    <div class="modal-body m-3" style="padding:0px;">
                        ...
                        <?php if ((strlen($this->session->userdata('passTok')) == 200) && ($this->session->userdata('EstAdmin') == 1)) { ?>
                            ...
                            <div style="width:100%; text-align: right;" id="idBlockButtonAddAudio">
                                <button type="button" onclick="open_add_audio();" class="btn btn-primary button-modal-login" data-toggle="modal" data-target="#selectAudioModal">Selectionner audio</button>
                                <hr>
                            </div>
    ...
    <script>
                        ...
                        <div class="row">
                            ...
                            <div class="col-sm-4" style="float:right; width:120px;">
                                <?php if ((strlen($this->session->userdata('passTok')) == 200) && ($this->session->userdata('EstAdmin') == 1)) { ?>
                                    ...
                                    <button  style="padding: 10px;" type="button" onclick="open_add_audio();" class="btn btn-primary button-modal-login" data-toggle="modal" data-target="#selectAudioModal">
                        ...
                        innerHTML += `
                         <div class="card-audio" name="`+directories[i].items[j].path+`" onclick="selecteAudio(this)">
                            ...
                            <div> 
                               ...
                               +`</p>
                               <i  onclick="deleteAudioFromServer('` + directories[i].items[j].path + `')" class="fa fa-trash" style="font-size:18px; color:white;" aria-hidden="true"></i>
                            </div>
                         </div>`
                       }
                    }
        ...
        function deleteAudioFromServer(audioPath) {
            if (confirm("Are you sure you want to delete this audio file?")) {
                // Make an AJAX request to the server to delete the audio file
                var xhr = new XMLHttpRequest();
                xhr.open("POST", "<?php echo base_url(); ?>video/deleteAudioFromServer", true);
                xhr.setRequestHeader("Content-Type", "application/json");

                xhr.onload = function() {
                    if (xhr.status === 200) {
                        alert("Audio file deleted successfully!");
                        // Optionally, you can refresh the audio list or remove the item from the DOM
                        ...
                        try {
                            ...
                            alert("Error deleting audio file: " + (response.message || "Unknown error"));
                        } catch (e) {
                            alert("Error deleting audio file: " + xhr.statusText);
                        }
                    }
                };
                ...
                xhr.send(JSON.stringify({ path: audioPath }));
            }
        }
