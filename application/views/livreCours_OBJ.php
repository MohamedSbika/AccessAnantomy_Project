				...
				<div class="card" style="height: 100%;">
					<div class="card-header">
						...
						foreach ($listFig as $value) { ?>
							...
							<a name="<?= $value['IDFigure']; ?>" id="setFigStyl" onclick="setFig('' ,'','<?= $value['IDFigure']; ?>')" class="btn" href="#" style="border-color: transparent;color: #000000;padding: .1rem .2rem;"><?= $value['TitreFigure']; ?></i></a>
		...
		function setFig(ur = '', titur = '', iFig = '') {
			...
			$.ajax({
				...
				success: function(html) {
					...
					if (ar[0]["id"] == 1) {
						ur = ar[0]["desc"][0]["encryptFigure"];
						titur = ''; // ar[0]["desc"][0]["TitreFigure"];
						$("#figZoo").html('<img class="zoo-item" id="zoom_01" src="data:image/png;base64,' + ur + '" />');
