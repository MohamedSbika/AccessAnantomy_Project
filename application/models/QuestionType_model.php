...
class QuestionType_model extends CI_Model {
    ...
    private function _get_datatables_query(array $d_arr = NULL)
    {
        ...
        if($resParams[0]["Value_Params"]< 100){
            ...
            $wrclause = ' 1=1 ';
            if($d_arr[0]<> ''){$wrclause = $wrclause." AND OperationType ='".$d_arr[0] ."'";}
            if($d_arr[1]<> ''){$wrclause = $wrclause." AND IDChapitre ='".$d_arr[1]."' ";}
            $this->db->where($wrclause);
            $numRow  = $this->db->get()->result_array();
            $limitRow 	= round((count($numRow) * $resParams[0]["Value_Params"])/100) ;
        }
